...
- [x] **v1.08 班期平衡 2.0(balance 2.0)**: v1.07 首轮实测(周日晚 0→32 班, 周六 909→931)证明管线正确但回填太慢——每(星期×方向)只探 1 个日期、offer 只取前 8 条、12h 节流。core/dow_balance.py 升级: 严重饥饿(低于峰值 20%)的星期每轮探测窗口内 3 个未来日期(候选池 6 个按 done 去重滚动续探), fetch_lowest 新增 offer_limit 参数让平衡探针拉全日约 30 条航班行(快照条目不受影响仍是前 8), 节流在存在严重饥饿星期时放宽为 4h, 恢复后回到 12h; 弱而不严重(20%~35%)的星期保持每日 1 个日期的温和节奏。absorb 只计真正新增的航班+星期对, 既有观察条目永不覆盖。tests/test_v108.py 10 断言(严重分级/多日期探测/offer_limit 传递/4h vs 12h 双节流/去重续探)全绿。
- [x] **v1.09 公务舱无密钥采集(keyless business-cabin patrol)**: 公务舱模块自 v0.66 起骨架完整(巡检腿/历史环/历史新低/告警冷却/书签回填吸收), 但 cabin_history.json 一直是空 routes——无 Amadeus 密钥时巡检只 standby, 采集从未真实发生; 去哪儿日历网关对舱位参数无响应(实测 8 种变体全失效), H5 列表页签名门禁未变。换攻击面: flights.booking.com LOWEST_PRICE 网关接受 cabinClass=BUSINESS, 纯 HTTP 无 key 可用(实测 CKG→HGH 公务 €741 vs 经济 €159, HGH→BKK €346, 含税总价+精确时刻+15 条 offer)。① core/booking_fill.fetch_lowest 加 cabin_class 参数(默认 ECONOMY 不变); ② core/cabin_monitor 新增 probe_dates(窗口内日期按最后观测时间排序: 从未探测优先→最陈旧优先, 每腿每轮 K 个, 零额外状态重启自动续)与 booking_cabin_rows(EUR×ECB 汇率−税费=裸价, total_price() 精确还原含税应付价); ③ main.cabin_patrol_once 重构: 书签回填吸收从无密钥分支提升为每轮必跑, Booking BUSINESS 无密钥探测每轮必跑(间隔沿用 call_interval≥4s), Amadeus 通道有密钥时叠加; ④ source=booking-cabin 不入 NON_REAL_SOURCES(真实可购报价, 直接进历史环与历史新低告警), /api/cabin 增加 booking_cabin_ready。tests/test_v109.py 9 断言: 参数透传+minPrice 解析、探测优先级(从未探测→陈旧优先→k 截断)、裸价+税费回环=含税应付、空值守卫、默认配置、booking-cabin 真源语义。
- [x] **v1.10 Agent 任务中心 + 公务舱精准化(agent task center & per-flight cabin timetable)**: ① 观测优先的任务中心——core/agent_tasks.build_ledger 把 7 类后台任务(全量扫描/公务舱巡检/班期平衡/每日备份/汇率刷新/健康巡检/洞察周报)从各自独立的状态文件投影成统一账本 {id, name, icon, status(ok/late/idle, 1.5x 节奏判 late, 时钟偏移算 ok), last_run, next_due, detail, manual_trigger}; webui 新增 GET /api/tasks 与 POST /api/tasks/cabin-patrol/run(手动点火公务舱巡检, 推送静默), 前端新增 🤖 任务中心标签页(卡片式账本+刷新+手动触发), 调度仍由现有 worker loop 负责——后续逐任务搬迁调度零 UI 改动。② 公务舱采集精准化(用户点名「尽快优先补齐, 要求精准」)——v1.09 每日期只记 aggregation.minPrice 一条匿名最低价, LOWEST_PRICE 响应里 ~15 条逐班报价(各自的税后 EUR 总价+精确起降时刻+航班号)全被丢弃; 现在 booking_cabin_rows 把整组 offers 逐班转 FlightDeal 入史(minPrice 降级为无 offers 兜底), record_low 去重键升级为 日期×舱位×航班号(多班共存/同班替换, fno-less 旧调用共享 "" 槽完全兼容), HISTORY_CAP 120→300, probe_dates_per_round 默认 6→12, offer_limit 3→8, history_board 榜单带 low_fno。实测 PEK→HGH 2026-10-20: n_offers=152, 8 班公务舱逐班入史 ¥5534.6~11349.2(HU7103 17:00 中转次日达 vs HU7577 06:55 直飞, 价差近半——v1.09 这类信息全丢)。tests/test_v110.py 9 断言 + tests/test_cabin_precision.py 7 断言全绿。
- [x] **v1.11 公务舱标签页(cabin tab: from zero)**: 排查实锤——公务舱后端自 v0.42 存在、v1.09 无密钥采集打通、v1.10 逐班精准入史, 但前端 9 个标签页(dash/crawl/agents/routes/reverse/weekly/sources/push/logs)里没有 cabin, index.html/app.js 里 "cabin/公务舱" 零命中——所有逐班数据用户在页面上完全看不到, 「部署太轻量」的直接实锤。本版从零建 👔 公务舱标签页: ① record_low 新增 dep/arr 持久化(v1.10 只存 fno, 时刻表无起降可言; 旧行随同班替换语义下一轮巡检自动补齐); ② cabin_monitor.history_timetable 纯函数——每腿取最便宜 N=8 条 fno 观测(价格升序/日期 tie-break), 腿按各自最低价排序, fno-less 旧观测仅入榜单不入时刻表; ③ /api/cabin 新增 timetable 投影, 服务端用 city_iata 拼 Booking 公务舱搜索深链(type=ONEWAY&cabinClass=BUSINESS); ④ 前端 cabin-status 状态条(最近巡检结果+「立即巡检」手动触发, 探测中转圈 2-3 分钟后自动刷新)+历史最低徽章(low+low_fno+low_date)+逐班时刻表(日期/航班号/起降时刻/含税总价, 整行可点直达 Booking), 样式全走设计令牌。tests/test_v111.py 4 断言: dep/arr 入环+legacy 兼容、投影排序/fno-less 剔除、per_leg 截断+空守卫、webui 三件套接线契约。
- [x] **v1.12 时刻质量热力图+公务舱走势图(time heat + cabin sparklines)**: 排查实锤——v0.81 就建好的 /api/time-coverage 逐日质量矩阵前端从未调用(app.js 里 "time-coverage" 零命中), 时刻来源构成只有 7 条汇总条形图, 用户看不到具体哪天是跨日借用、哪天能自动转正; 240 deals 快照实况: exact 196(booking 144+机场板 52)+borrow 42(booking-x 跨日借用)+alt 2, 零缺失行, 主诉剩余=42 行借用+2 行参考的可监督性。本版: ① 时刻覆盖体检卡内新增 timeHeat 容器(顺手删除误加的 tab-timeheat 空 section), loadTimeHeat/renderTimeHeat 每条监控线路一行——腿名+四色计数徽章(精确/借用/参考/无参考中文)+逐日格子条(exact 绿/borrow 蓝/alt 琥珀/noref 灰, title=日期 班次 起降时刻, 借用格带 promote_on 自愈转正日), 随 renderTimeCoverage 既有三处调用点自动刷新; ② cabin_monitor.history_timetable 每组新增 spark 投影——该腿观测按日期取 min 价格、日期升序、截最新 30 点(fno-less 旧观测有真实价格, 计入); ③ 前端 cabinSpark: SVG polyline 120x28, min-max 归一化, 最低点绿点标记+「近N日 ¥lo–¥hi」标签, 插在公务舱腿卡头部下方; ④ 样式全设计令牌(th-cell 四色族/徽章 pill/sparkline stroke 走 accent)。tests/test_v112.py 5 断言: spark 按日 min+升序+fno-less 计入、30 截断保最新、空历史守卫、webui 三件套接线契约(timeHeat 容器/loadTimeHeat+cabinSpark/css)、误加 section 移除。
- [x] **v1.13 v2 主入口补齐任务中心+公务舱逐班表(v2: agents + cabin timetable)**: 排查实锤——v1.10 任务中心与 v1.11 公务舱逐班时刻表只落在经典版 /classic, 用户默认打开的 v2 主页(Preact) TABS 里没有 agents/cabin, dump 实测 hash 回落仪表盘(三个 tab bytes 相同 57853), 「部署太轻量」的延续。本版: ① web/src/components/AgentsView.jsx 从零建——fetchTasks 账本投影, AgentCard 状态徽章(ok 绿/late 琥珀+浅底/idle 灰)+detail+上次(距今格式化)/节奏/下次 三段 meta, manual_trigger 手动点火(scan-all 走 /api/run 其余走各自 trigger, busy 转圈完成后自动刷新账本); ② web/src/components/CabinTimetable.jsx 从零建——消费 /api/cabin 的 v1.11 timetable 投影: 每腿一卡(腿名+SparkPts 走势), 逐班四列表(日期/航班号/起–降/含税总价), 整行点击 window.open Booking 公务舱深链; SparkPts 用 v1.12 后端 spark 投影(按日 min/升序/30 点/fno-less 计入)替代旧本地 slice(-10)——数据口径与经典版完全一致; ③ CabinCard 非编辑模式尾部接线 <CabinTimetable>, app.jsx TABS 加 agents(第5位)——v2 现在 9 个功能页; ④ 样式全走 tokens(app.css 追加 v1.13 块: agents-grid 自适应网格/agent-card 状态配色/cab-tt 腿卡/hover 行高亮), npm run build 产出新 bundle(index-Djm1FoKa.js 132.93kB)。tests/test_v113.py 3 断言: agents 三件套接线(app.jsx TABS+import+render/组件+api.js 导出)、cabin 三件套(CabinTimetable 消费 timetable+spark+深链/CabinCard 接线)、构建产物新鲜度(dist 引用+css 含 .agents-grid)。
- [x] **v1.14 公务舱时刻缺口优先回查(time-gap-first probe + coverage chip)**: 根因实锤——probe_dates 只按最后观测时间排序, 某日若历史上只拿到过匿名 minPrice 兜底行(无航班号无时刻), 它按 ts 显得『新鲜』永远排不到探测队列前——时刻格永远空着。本版: ① core/cabin_monitor.py probe_dates 排序升级为 (pri, ts, date): pri=0 从未探测 / pri=1 最新观测批无 dep+arr(time_first=True 时插队) / pri=2 常规最旧优先, time_first=False 保留纯 v1.09 口径; _newest_timed(route) 按日取最新批 ts+是否有任一行带起降时刻(同批共享 ts); ② time_gap_dates() 纯函数圈定窗口内缺口日, main.py cabin_patrol_once 每腿 gaps 交集计数 n_refill→info.time_refill, last_status 追加『· 时刻回查 N 日』(任务中心卡片直接透出); ③ history_timetable 每腿新增 timed/total(展示行的满时刻计数), v2 CabinTimetable 每腿头部渲染『时刻 x/y』覆盖芯片(满覆盖绿 cab-tt-cov.full, 悬停说明回查策略), app.css 追加 v1.14 芯片样式全走 tokens; ④ 推送通道排查确认 _cabin_absorb 已接 push_all(kind=cabin-record/cabin)+冷却, 无需重复接线。补充修正: 缺口判定含两类——a) 最新观测批无时刻(匿名兜底行), b) 当日最便宜 fno 行无时刻(旧书签行按价格永远压住新带时刻观测); 预算切分: 全新 60 天窗口的几十个从未探测日会占满整个 k 预算饿死缺口修复, 现每轮最多一半名额(k//2, 至少 1)优先给缺口日, 其余继续轮转。 history_timetable 按 (日期,航班号) 去重, 带时刻观测升级同航班无时刻孪生行(否则回查永远不可见)。 tests/test_v114.py 9 断言: 无时刻日插队(never→gap→stale)/time_first=False 旧口径保留/同批带时刻行判定/缺口日窗口过滤/timetable timed+total/组件芯片接线/巡检计数接线。
- [x] **v1.14.1 cabin_refill 一次性缺口补查运维工具(one-shot gap refill)**: v1.14 的 k//2 预算切分每 30 分钟一轮只修几个缺口日, 全补完要等数小时; 本版 tools/cabin_refill.py 把『缺口日精点能查到』(用户原话)变成一条命令立等可查——按巡检同一条数据通路(Booking LOWEST_PRICE BUSINESS → booking_cabin_rows → _cabin_absorb, 推送关闭+空 state dict)一次性补完全部时刻缺口日, 只写 cabin_history.json(原子写, 不碰 state.json, 与在跑 worker 并发安全); --dry 只列缺口清单不发请求, --max N 控制每腿上限(默认 20); 输出严格区分 transient(非 200/限流, 稍后重试)与 server-confirmed no offers(服务器确认无票)两种空结果, 与 v0.88.1 语义对齐——实测验证: 工具跑通 17 缺口日全部 transient(裸调证实网关 429 IP 级限流, 换 UA 无效), 输出不误读为无票。ui_check v1.14.1 断言工具存在+双语义标记。
- [x] **v1.15 Booking 网关 429 熔断+会话指纹轮换(throttle circuit breaker + fingerprint rotation)**: 实锤——连续两轮巡检整轮零报价+17 缺口日补查全空, 裸调抓包证实网关 429『Rate limit exceeded』(IP 级, 无 Retry-After, 响应还下发新 cookie, 限流键=IP+cookie+UA 组合); 旧代码非 200 一律 None, 调用方把限流当普通瞬时错误继续打满整轮探测, 拉长封锁窗口。本版: ① core/booking_fill.py fetch_lowest 429 显式返回 {"throttled": True}(区别于 no_data/None 三态语义), UA 指纹池 current_fingerprint(config 钉死 UA 作池首, idx=0 字节级保留旧行为)/bump_fingerprint(模块态轮换, 无 IO)/warm_session(GET 搜索页暖 cookie, 429 响应也发 cookie); ② 公务舱巡检循环(main.py cabin_patrol_once)遇 throttled: 每次 bump+重建会话+重暖, 两击熔断当腿剩余探测, info.throttle 透出, last_status 追加『网关限流熔断xN』——任务中心卡片直说限流而非无票; ③ 经济舱 fill_gaps 同样两击熔断(限流不写负缓存不污染轮转游标, stats.throttled 透出); ④ tools/cabin_refill.py 遇 429 熔断当腿并提示 rerun later。 tests/test_v115.py 8 断言: 429→throttled/503→None 保留/throttled 不产生报价行/idx0=config 钉死 UA 兼容/bump 轮换/warm 命中搜索页/fill_gaps 两击停+零负缓存污染/巡检+工具接线。
- [x] **v1.15.1 429 限流自适应退避(adaptive patrol backoff)**: v1.15 熔断止住当轮空转, 但 worker 仍按固定 30 分钟间隔下一轮继续撞墙。本版给退避加上自适应: ① core/cabin_monitor.py patrol_gap(base_seconds, prev_streak, throttle_hits) 纯函数——本轮再限流则 streak+1, 下一轮间隔 2×/4×/6× 封顶(30→60→120→180 分钟), 干净一轮立即复位 base; ② main.py 巡检开头读上轮 throttle_streak, 结尾写 info.throttle_streak/interval_effective_minutes, last_status 追加『限流退避下轮+N分』, worker 循环 next_patrol 改用 effective 间隔(异常回退 patrol_gap); ③ 任务中心卡片 cadence 跟随 interval_effective_minutes, 退避中不再误报 late; ④ /api/cabin 投影透出 throttle_streak/interval_effective_minutes。 tests/test_v1151.py 7 断言: 干净轮复位/首熔断 2×/二连 4×/三连 6× 封顶/卡片退避中节奏/干净轮回 base/端到端接线。
- [x] **v1.23 公务舱时刻按需精查(on-demand route fill)**: 后台巡逻每轮只补 4 班且 429 自适应退避最长 180 分钟, 用户盯着的腿页面缺口却等不起——把 v1.22 的携程 CDP 精查能力从后台配额里解放出来做成按需: ① core/cdp_board 新增 fill_route(route_targets) 单航线定向靶点——该腿 timeless 观测按 fno 去重/最早日期优先, 红线与后台完全一致(6h 行缓存/24h 负缓存/共享账本每日 24 次硬上限/连续 2 败熔断), changed_db 原子写回; ② webui 新增 POST /api/tasks/cdp-fill/run(body {from_city,to_city,max_fnos?,force?}), exact>0 时回写 cabin_history, 返回逐班 rows(fno×date×起降×how 溯源); ③ 公务舱标签页每条腿卡片新增『补齐时刻』按钮——防重入/状态行轮播/成功后整面板自动刷新, 页面缺口 3-5 秒变精确起降时刻; ④ 与巡逻共享同一账本互补不抢占, 超限时诚实报 capped 而非越权; ⑤ 上库通道实弹——出口网络对 github.com:443 TCP 重置(api.github.com 正常), git push 连挂 4 次后复活 v0.84 的 tools/api_push.py git-data API 通道完成推送, API ref 更新实测会触发 push 工作流(CI 自动三连绿)。实弹 3 查 1 沉淀(CZ8885@10-08 大兴 16:00→虹桥 18:20 入库)。tests/test_v123.py 8 断言(城市过滤/最早优先/已定时跳过/全负缓存短路/熔断/限流截断/正常回写落盘)全绿。
- [x] **v1.24 公务舱每日最低价趋势面板(daily-lowest trend)**: 公务舱标签页每条腿新增宽幅走势卡——SVG 渐变面积线(v0.28 令牌配色)+网格刻度+心理价位阈值虚线(cabin_watch.threshold_total 落在量程内才画)+最低/最高极值点+悬停十字线带日期价格读数, 数据复用 v1.12 spark 序列(每观测日最低含税总价, 近30观测日), 纯前端零新接口; history_timetable 行 tsrc 缺省时按时补 patrol 溯源(有时刻的行不再裸奔); tests/test_v124.py 5 断言(前端面/CSS 令牌/缓存版本/tsrc 缺省/版本号)全绿, 全量 474 OK。
- [x] **v1.22 公务舱时刻携程板补全(ctrip CDP board)**: 上海板 timeDays=-1|0|1 物理够不到的远期缺口日与无机场板腿(重庆→上海整腿 20 号全缺)需要『按日期精查』数据源——大兴官网瑞数盾/首都机场无板/重庆板防护重定向/飞常准授权墙逐一实证排除后, 携程航班动态详情页 URL 直达任意未来日期(实测 HO1254@2026-10-25 大兴T2 21:25→浦东 23:40、3U8971@2026-10-16 江北T3 07:15→浦东T2 09:40, 数据源飞友)。本版: ① core/cdp_board.py 真实 Edge/Chrome CDP 拉起最小化有头窗口(无头被鲸护拦截), 单页加载提取 flight-detail-header 与 flight-detail-body-left 文本得航司/航班号/计划起降/城市/航站楼, 双航站楼布局按行归类; ② 红线与上海板一致: 6h 行缓存+24h 负缓存(仅成功渲染但无数据时落)+共享账本每日 24 次硬上限(失败也记账)+连续 2 失败熔断; ③ earliest-first 靶点, own-dow 沉淀(airport-board 权威/ctrip-detail 同层刷新); ④ patrol 挂 cdp_fill, /api/cabin 透出, 面板新增『携程板时刻』统计行。实弹 2 查 2 沉淀(CZ8879 12:00→14:10、HU7605 15:35→17:45)。tests/test_v122.py 13 断言全绿。
- [x] **v1.21 公务舱时刻价值优先调度(sh board value-first slots)**: 上海板 timeDays 只认 -1|0|1, dow_targets 却按最早观测日排序让『天天飞缺 7 星期』的航班主任永占槽位, 缺口恰在今天/明天的航班(HO1258/1260 类)永远排队。本版: ① 缺口与今明窗口无交集的号直接跳过(查了也沉淀不了任何新行); ② 可完成(全部缺口都在窗口内)的号优先, 缺行少者优先, 腿覆盖向 8/8 收敛; ③ sh_fill 只查有用的偏移(目标缺星期∈该偏移日期的星期), 单轮预算减半; ④ stats.dow_new(从未实现)移除, 新增 filled(实际沉淀行数), 面板统计行『航班 N』改『时刻行 N』。tests/test_v121.py 8 断言全绿。
- [x] **v1.20 公务舱时刻预算守护(sh board cap absolutism + neg cache)**: v1.19 实弹暴露越权——force=True 把每日硬上限也一起绕过, 账本烧到 32/24; CZ3440/CZ3480 这类板上查无此班的号继续每天占计划槽位。本版: ① fetch_flight 的每日 24 次上限改为绝对执行, force 只跳过 6h 行缓存(职责回归『刷新旧行』而非『超越预算』); ② success 空 flightList 落 24h 负缓存(fetch_flight 新增 neg-cache 返回, 零网络零预算), neg_hit 只读探针, dow 目标过滤掉双偏移全负的号——计划槽位让给真航班(MF8561/CZ8899 等), 起飞时刻覆盖持续提升; ③ /api/cabin 经 state 整体透传 sh_fill.neg, cabin 面板状态条新增上海板统计行(查询/精确回写/航班/负缓存/额度满/WAF 熔断)。tests/test_v120.py 7 断言全绿(上限绝对性/force 刷新语义/负缓存 TTL/槽位腾退/前端渲染)。
- [x] **v1.19 公务舱时刻预算精准化(sh board budget precision)**: v1.18 上线后实弹复盘——CZ3440/CZ3480 在上海板两个方向×两个日期偏移全部 0 行(疑似共享号/非两场起降), 但每天仍烧 4 次查询额度; 北京→上海腿卡 2/8, 因 CZ8803 的 10 月缺口日期坐在 6h 缓存命中区里, 而 CZ8879(12:00 起飞班)先烧掉了预算——远期日期插队, 最近能买的票反而等不到时刻。本版: ① dow_targets 按最早观测日期升序排序(远期日期不再饿死近程缺口); ② sh_fill/fetch_flight 增加 force 透传, tools/sh_cabin_fill.py 加 --force 一次性绕过 6h 行缓存强制重查(仍受每日 24 次上限保护); ③ 跨零点到达标记——apply_exact_times 检测 arr<=dep 写 cross_day, cabin timetable 行投影透传, UI 行时刻带『+1』徽标(22:25–22:10 一眼看出次日落地, 非负时长); ④ v2 构建产物同步重打包。tests/test_v119.py 5 断言全绿。
- [x] **v1.18 公务舱时刻接入上海机场官方板(sh board exact cabin times)**: 用户点名公务舱模块「尽快补齐, 数据源要求精准」——萧山机场板天然看不到上海两场起降的航班, Booking 网关又连日 429 封锁, 北京→上海腿时刻表长期 2/8 缺时刻。本版接入上海机场(集团)官网 Avinex 航班板(与浏览器同源, 零密钥零签名): ① core/sh_board.py fetch_flight——POST OldFlightHandler.aspx 按航班号+方向+日期偏移(-1|0|1)直查官方计划起降/城市/航司/候机楼, 6h 缓存+全局每日 24 次硬上限(与萧山板共用记账, 失败也烧额度); ② exact_targets 圈定今/明窗口缺时刻上海腿观测, apply_exact_times 按 日期×航班号×城市对 契约原地回写(tsrc=shanghai-board, 已有真实时刻永不覆盖); ③ merge_sh_rows 按自身星期沉淀进 flight_sched_db(萧山 airport-board 行保持权威只填空, 同 tier 最新覆盖), dow 补齐后查询自熄灭, 远期日期由 v1.16 借用契约消费; ④ cabin_patrol_once 每轮挂钩 sh_fill, /api/cabin 透出统计, UI 行带『上海板』徽标; ⑤ tools/sh_cabin_fill.py 一次性补查运维工具。实测 HO1254 相邻日期到达 23:35 vs 23:30——按日精确>按星期借用。tests/test_v118.py 16 断言全绿。
- [x] **v1.17 公务舱推送带起降时刻(cabin push dep-arr times)**: 公务舱低价提醒此前只报『日期+价格』, 用户要求精准——推送必须直接看到航班号与起飞/落地时刻, 免去回面板二次确认。本版: ① core/cabin_monitor.py evaluate_alert 命中项透传 fno/dep/arr(历史记录早就存了, 只是没带给通知层); ② 新增 push_time_suffix(row, sched, from, to) 纯函数——时刻优先取观测自身, 缺时刻的行按 v1.16 三方精准契约(航班号+起飞星期+城市对)从排班库现场借用, 借不到返回空串保持干净文案, 共享号取首段, 'HHMM' 自动补冒号; ③ main.py _cabin_absorb 两条推送路径(历史新低/阈值)统一追加『· 航班号 HH:MM–HH:MM』后缀, 排班库仅在真正触发推送时懒加载(零推送零开销)。 tests/test_v117.py 7 用例: 自身时刻带航班号/缺时刻三方借用/城市不匹配拒绝/共享号首段/无可信时刻留空/阈值命中透传字段/端到端接线。
- [x] **v1.16 公务舱时刻借用(cabin sched time borrow)**: 公务舱时刻表大量行长期『时刻待采』——timed 补全走 Booking 网关(连日 429 封锁), 而经济舱侧的零密钥排班库(机场板实测, 3768 班, 每天更新)早就存着同一批航班的星期制时刻。本版把库借过去: ① core/cabin_monitor.py borrow_sched_time(row, sched, from, to) 纯函数——fno+起飞星期+城市三方全匹配才填(不匹配不借, 精准优先), 共享航班号 'CZ3502/CZ2326' 取首段, 已有真实时刻的行永不覆盖, 命中行标 tsrc=sched-borrow; ② history_timetable/history_board 增加可选 sched 参数, top-N 行与历史最低行原地升级 dep/arr, leg 透出 borrowed 计数与 timed 复算; ③ webui /api/cabin 加载 flight_sched_db 传入两处投影; ④ UI: 借用行『借用』徽标(hover 说明来源与购票页为准), 卡片头部『时刻 x/y · 借用z』进度章。 tests/test_v116.py 10 断言: 三方匹配填时刻/城市不匹配拒绝/星期缺口拒绝/共享号首段/真实时刻不覆盖/时刻表借用+计数/无库保持原样/board 最低行升级/webui 接线/UI 徽标。
- [x] **v1.07 周末班期自动平衡(dow balance)**: 机器周末关机导致板库永远缺周六/周日沉积(实测 dow0-4 各 3000+ 班, dow5=909, dow6=0), 周末出发的航班时刻长期停留在弱参考钉——而用户手动精点查询其实都能查到(精点端点一直活着)。core/dow_balance.py 每轮扫描统计七天覆盖, 发现饥饿星期(低于峰值 35% 或绝对地板 60)就对未来 +2..+25 天内最近的该星期日期, 走既有 fetch_lowest 报价管道拉一次当日全部 offers, 映射进 sched_deposit 队列并当场吸收——每行按自己的星期归档, 既有观察条目永远优先(absorb 只填空), 12h 轮节流 + 同 dow+日期+方向 7 天去重 + 异常全捕获绝不影响主扫描。周末出发的起飞时刻由此自动升级为 起飞·班期精查, 无需周末开机、无需新数据源。tests/test_v107.py 18 断言(检测/窗口日期/映射/吸收/不覆盖/节流/失败安全)全绿。
- [x] **v1.06 多段联程首段时刻升级 + 航班号复用防护**: 中转行程(如 CZ3502/CZ2326 杭州经广州转重庆)的起飞时刻此前沿用 Booking 同日最低价借用钉——可能是另一条行程的时刻, 实测偏差可达 5 小时以上(CZ3502 真实 18:35, 借用钉 13:10)。现在首段航班号+星期在出发机场板库精确命中时, apply_board_upgrade_first_leg 只升级起飞时刻为该班机真实时刻(起飞·班期精查 徽标), 落地与中转信息保持诚实标注(落地侧徽标独立)。同时修复航班号复用陷阱: SC2118 周三实为华夏航空 乌鲁木齐→杭州、其余天为山东航空 杭州→厦门, board_lookup_x 精确命中前先校验出发机场侧, 不同机场的同号班机不再误当同一班——快照里「中转 杭州」这类荒谬数据清零(实测 33 条 booking-x 多段行: 21 条升级首段精查、10 条降级跨日参考、2 条参考班次, 复用污染 0 残留)。tests/test_v106.py 14 断言 + 既有 test_sched_board 语义更新(经停宽容仅限到达侧)全绿。
- [x] **v1.05 精点回填 v2: 书签抓全列表 + 日详情就地回填**: 用户确认「灰色日期直接精点能查到数据」——qunar 日历服务端无缓存价(非售罄), 单日列表 API 被 Bella 签名+指纹风控拦(自动化三路皆堵: 无头/嵌入浏览器/CDP, 当日复测防御 JS 仍在), 真人浏览器是唯一通路, 于是把通路翻倍提效: ① bookmarklet v2 卡片级抓取——价格节点向上爬祖先找「航班号+HH:MM」最深容器, 每班航班号+起降时刻+价格全量入 rows(单日多班一次 POST, 上限12班, 抓不到降级 v1 最低价再降级手输); ② put_rows 同日多行取最低价入缓存槽(原语义后写覆盖, 高价会顶掉低价), 全量行照常喂班期沉积队列(一次精点教会全线当日时刻); ③ DayDetail 参考价日期(booking-ref/interp/nearby-ref)底部就地回填表单(付款价必填+航班号/起降选填, postPointFill 复用), 不用再跑去「数据抓取」页找日期; ④ CrawlView 书签文案升级 v2。新增 tests/test_v105.py 13 断言(最低价胜出/裸价口径/booking-ref让位/真价不动/已回填日期退出gap列表/书签ORIGIN绑定+卡片标记+node--check语法/前端接线)。
- [x] **v1.04 真价行时刻升级: 精确板期压过 Booking 借用钉**: 用户持续推进「页面要显示航班班次具体起飞时间」——实测发现 43 个 qunar 真价航班号里 42 个在 3758 班的机场班期库里有班期(GS6582/HU7421/3U2581 等 dow 0-4 全满), 但 booking_attach(v0.88) 先把同日 Booking 最便宜行程钉成 booking-x, 之后板库 attach 因「dep+arr 都已有值」被跳过, 真价行 94 天里 66 天一直卡在借用时刻。修复: ① core/sched_board.apply_board_upgrade 纯函数——仅当行是 booking-x 且板库精确命中(dow 命中, 机场维度唯一=同一物理航班)时, 用本航班号时刻覆写并清空 borrow 溯源, 价格与航班身份永不动; ② _enrich_flight_times 单段路径增加 bk_borrowed 升级分支, 跨星期板期(exact=False)永不覆写 booking-x(同日他航班仍优于错星期); ③ booking_fill EXACT_SOURCES 含 airport-board 合同上锁, 升级后下一轮 Booking 钉子自动让位, 无乒乓; ④ crawl step 文案带 (bk-upgrade xN)。预期一轮后真价行 dep_exact 0→约 66, 剩余 28 天=周五(库 909 班)/周日(0 班, 09-20 自愈)板库缺口。新增 tests/test_v104.py 5 断言(仅 booking-x 可升级/升级后三源改 airport-board+清 borrow/无 dep 不升级/合成库 exact vs 跨 dow 策略链/无乒乓合同)。
- [x] **v1.03 时刻口径修正 + CDP 负结论归档**: 用户确认「灰价日并非售罄, 精点有价」后复盘时刻统计, 发现快照 240 行中 144 行带 Booking 同日真实行程时刻(dep_src=booking: 重庆 23/郑州 43/曼谷 58/成都 20), 但 time_coverage 只认 amadeus/airport-board 为精确、又把 NON_REAL(booking-ref) 行整体排除出分母, 制造「精确时刻 0 天」假象; /api/time-coverage 同病——booking/booking-x/point-fill 全落 noref 灰格, 日历热力条 144 天显示「无时刻」。修复: ① EXACT_TIME_SOURCES 纳入 booking/point-fill, 纯函数 time_kind(dep_src) 统一分类(exact/borrow/alt/noref, booking-x 算借用), 前后端共用; ② time_coverage 新增 ref_total/ref_dep_exact/ref_dep_borrow/ref_dep_missing 参考行计数, 口径透明; ③ 日历图例改「当日真实时刻/参考借用时刻/无时刻·待精查」, 数据源页时刻沉淀卡新增参考行计数行; ④ CDP 自动精点负结论归档 point_fill.py——Chrome 136+ 默认 profile 忽略 --remote-debugging-port(接管真实 Chrome 不可能), 独立 profile+iPhone 模拟过登录墙但列表页软拦「没有查询到符合条件的航班」, Trip.com HTTP 000; ⑤ Booking 按日轮转健康实证(最近 run 探 0·缓 0·补价 23·补时刻覆盖全部灰价日), 真价自动补齐唯一待办仍是 Amadeus 免费密钥。新增 tests/test_v103.py(time_kind 五映射 + ref_* 计数三态)。
- [x] **v1.02 自动精点引导 + 班期缺口周报**: 用户确认「缺数据源日期直接精点能查到」后复盘发现两条补齐通道都没真正运转——Amadeus 轮转精点(v0.67)因密钥未配每轮空转 skip, 书签回填缓存 0 行从未用过, 于是 143 个灰点日只能等 Booking 参考价。本版把「配免费密钥即全自动补齐」做成显式引导: ① /api/point-gaps 新增 auto_fill 块(amadeus_ready/per_round=6/interval_min/eta_rounds/register_url), 精点面板横幅两种状态——未配密钥时直达 developer.amadeus.com/register 免费注册(个人邮箱即可, 之前卡住的是 enterprise 销售页)并给「配好后每轮自动精点 6 日」预期, 已配时显示激活态与补齐轮数预估(缺口最多路线 ceil(N/6) 轮); ② 数据源页 Amadeus 区新增三步注册指引 + 「测试密钥」一键校验按钮(调 /api/amadeus-test); ③ 周报正文末尾新增班期库沉淀行——sched_stats 统计 7 个 weekday 覆盖, 缺口日给出「常驻运行下 M/D 自动补齐」预测(板库每日沉淀当日+次日 dow), 本机实测「6/7 缺周日 → 09/20 自愈」; ④ build_weekly 加 data_dir 参数, webui 两个周报入口与 worker 周报推送全链路透传。新增 tests/test_v102.py 4 断言(auto_fill 契约 keyless/armed + dow 行三态: 无库诚实 0/7、全满沉默、周日缺给自愈日期)。
- [x] **v1.01 灰点作战清单修复(booking-ref 误判)**: /api/point-gaps 的 gap_dates 只认 interp/nearby-ref 为缺价日——v0.87 引入 booking-ref 后, 交叉补价盖住的日期被误判为「已有真价」, 四条线路 143 个灰点日(杭州-重庆 23/郑州 41/曼谷 59/成都 20)在精点补查面板里显示「无缺价日」, 而这些日期 OTA 单日精查实际有价(服务端聚合日历无缓存价≠售罄)。修复: gap_dates 判定统一走 NON_REAL_SOURCES(interp/nearby-ref/booking-ref), 主爬虫 Booking 轮转不受影响(它用独立 covered 集合选日期, 零副作用)。实弹佐证: 内置嵌入式浏览器访问去哪儿单日列表也被软拦(返回「没有查询到符合条件的航班」), 印证 v0.76 侦察「无 key 自动精点不可行」——精点回填必须真人真浏览器(Chrome/手机书签), 修复后面板每个灰点日恢复「点日期直达精查→点书签自动回填」闭环。test_booking_fill 语义断言同步修正(booking-ref 日=靶子)。
