...
监控未来 N 天 **机票最低价(含机建燃油,即最终支付口径)**,低于你的心理价位就推送到 iPhone;同时对比 12306 动车票价(含学生票 75 折估算),帮你选出**出行最优方案**。自带 Web 仪表盘,单文件依赖极简,适合部署在私人设备上长期运行。
...
- 🚄 **动车对比**:12306 票价接口(免登录),自动标注二等座与学生票 75 折估算,识别夕发朝至车次
- 🧳 **托运行李标注**:按航司标注是否含免费托运,廉航/特价舱自动加 ⚠️ 提示
- 📊 **Web 仪表盘**:60 天价格热力日历、价格趋势图、"出行最优"三卡片对比(机票/动车/学生动车)、配置全可视化编辑
...
| 12306·票价查询 | ✅ 可用 | 免登录低频查询,结果缓存 24h;学生票接口不区分,按二等座公布价 75 折估算(偏低参考,以下单页为准) |
...
- [ ] **v0.3 拓展**:多源比价(航司官网/携程 H5)、往返/多旅客、价格预测(历史序列)、微信推送模板优化
...
- **学生票准吗?** 12306 查询接口不区分学生价,这里按二等座公布价 75 折估算(略偏低),优惠区间/资格以下单页为准。
